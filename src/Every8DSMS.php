...
class Every8DSMS
{
    ...
    public function send($message = '',$phone = '',$subject = ''){
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $every8D['Api_url'].'sendSMS.ashx?UID='.$every8D['account'].'&PWD='.$every8D['password'].'&SB='.urlencode($subject).'&MSG='.urlencode($message).'&DEST='.$phone);
